...
into the filing_metrics MySQL table.
...
def load_segments(conn, ticker, year, doc_type, path, stmt_type):
    ...
    with conn.cursor() as cur:
        cur.execute(
            "DELETE FROM filing_metrics WHERE ticker=%s AND fiscal_year=%s AND doc_type=%s AND statement_type=%s",
...
def load_financial_ratios(conn, ticker, year, doc_type, path):
    ...
    with conn.cursor() as cur:
        cur.execute(
            "DELETE FROM filing_metrics WHERE ticker=%s AND fiscal_year=%s AND doc_type=%s AND source='financial_ratio'",
            (ticker, year, doc_type)
        )
        deleted = cur.rowcount
        cur.executemany(INSERT_SQL, rows)
