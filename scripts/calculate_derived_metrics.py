...
values already in filing_metrics, then appends them back to the
...
# ── Metric resolver: find a value from the filing_metrics DB ─────────────────
def get_metric(conn, ticker: str, fiscal_year: int, doc_type: str,
    ...
    for label in label_candidates:
        result = conn.execute(text("""
            SELECT numeric_value FROM filing_metrics
...
def calculate_for_filing(conn, ticker: str, fiscal_year: int, doc_type: str):
    ...
    Each entry is ready to INSERT into filing_metrics and append to JSON.
    """
    def g(*labels):
