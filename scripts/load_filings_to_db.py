"""
Load filing metrics from FINAL_FACTS_FILTERED.json into MySQL filing_metrics table.
...
INSERT_SQL = f"""
    INSERT INTO filing_metrics ({', '.join(DB_COLUMNS)})
...
def load_filing(conn, ticker: str, year: str, doc_type: str, json_path: str):
    ...
    with conn.cursor() as cur:
        ...
        cur.execute(
            "DELETE FROM filing_metrics WHERE ticker=%s AND fiscal_year=%s AND doc_type=%s",
